...
while z < meanBfrac:
    ...
    while i<meanAfrac:
        ...
        for row in range(SP):
            for column in range(SP):
                coin = np.random.random()
                if coin<CONNECTIVITY:
                    A[row][column] = np.random.lognormal(mean=np.log(meanA), sigma=np.log(varA), size=None)
        if z==0:
        ...
        for j in range(0,reps): #Perform experiment many times
            ...
            if diff > 8*SI_threshold: # if Similarity index is higher than threshold, the state is not stable
                num_unst+=1
